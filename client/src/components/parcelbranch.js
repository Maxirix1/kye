...
const ParcelBranch = () => {
    ...
    const fetchParcelsSave = async () => {
      try {
        const response = await axios.post(
          "http://kye.maxirix.com/api/parcelBranch",
          {
            username: username,
          }
        );
  ...
  const handlePaste = async (e) => {
    ...
    if (pastedData.length >= 5) {
      try {
        const response = await axios.post(
          "http://kye.maxirix.com/api/parcel/search",
          {
        ...
        try {
          const saveResponse = await axios.post(
            "http://kye.maxirix.com/api/parcel/saveerror",
            {
              id_parcel: pastedData,
              username: username,
            }
  ...
  const handleReceive = async () => {
    ...
    try {
      const responseUpdate = await axios.post(
        "http://kye.maxirix.com/api/updatereceive",
        { id_parcel: idParcel }
      );
